...
##############################################################################################################
#
# Safe is supposed to be a sealed environment, but we need to be able to monitor it status.
#   => this is Jean's solution : a webcam has been sealed into SAFE's cube, pointed at the monitor
#      access to this "monitoring interface" is restricted.
#   => this feature does not seems to be well documented, but it's there, everythings printed on stderr
#      will be visible on the monitoring.
#
def print_to_monitor(s, end="\n"):
    print('  '+s, file=sys.stderr, end=end)

# monitoring stream is quite crappy, add A LOT OF crc32 to fix video corruption if needed
def hexdump(data):
    length = len(data)
    sep = '-'
    for i in range(0, length, 16):
        ...
        print_to_monitor(f"{offset:04x} {hex_crc:08x} {hex_part} {ascii_str}")
##############################################################################################################
...
#               an archive may contains messages with different privileges (for ex. status and disarm)
#               we need to maintains sessions, a session is always closed before starting another
...
def tcp_send_receive(sock, req):
    ...
    # print_to_monitor("\n=================================================")
    # print_to_monitor("QUERY sz:%04X"%len(req))
    # hexdump(req)
    # print_to_monitor("=================================================\n")
...
#TODO 
def process_update_sig_bin(data):
    logging.warning("Not implemented")

#TODO 
def process_update_user_db(data):
    logging.warning("Not implemented")

##############################################################################################################
# DEBUG/MONITOR :
def process_utils_clear_screan(data):
    print_to_monitor("\033[H\033[2J", end='')
...
def process_utils_sleep(data):
    ...
    print_to_monitor("sleeping %ds"%sleep_time)
    time.sleep(sleep_time)


#   this message is used when testing end-to-end archive flow
def process_utils_get_flag_3(data):
    with open('/home/diode/crypto/flag.txt', 'r') as f:
        print_to_monitor(f.read())
##############################################################################################################
